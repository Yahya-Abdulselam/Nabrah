...
export function TriageResult({ data, whisper, wer, agreement, questionnaireResult }: TriageResultProps) {
  ...
  const handleAddToQueue = async () => {
    if (addedToQueue) {
      console.log('[TriageResult] Already added to queue:', addedToQueue);
      toast.info('Patient already in queue', {
        description: `Patient ID: ${addedToQueue}`
      });
      return;
    }

    console.log('[TriageResult] Adding patient to queue...');
    setAddingToQueue(true);

    // Show loading toast
    const loadingToast = toast.loading('Adding patient to queue...');
    ...
    try {
      ...
      console.log('[TriageResult] Calling addPatientToQueue with data:', {
        triageLevel: data.level,
        score: data.score,
        hasWhisper: !!whisperData,
        hasWer: !!wer,
      ...
      console.log('[TriageResult] Successfully added to queue:', response.patient_id);

      // Create patient object for Zustand store
      const newPatient: Patient = {
        id: response.patient_id,
        name: response.patient_id, // Use ID as name for now
        age: 0, // Not available in current data
        triage_level: data.level,
        score: data.score,
        confidence: data.confidence,
        timestamp: data.timestamp,
        features: data.features as any,
        analysis_summary: data.message
      };
      ...
      queueSync.broadcastPatientAdded(newPatient);
      ...
      toast.success('Patient added to queue!', {
        description: `Patient ${response.patient_id} is now in the queue.`,
        action: {
          label: 'View Queue',
      ...
      console.error('[TriageResult] Failed to add to queue:', error);
        ...
        {addedToQueue && (
          ...
          >
            <Button
              variant="link"
              onClick={() => {
                // Force full page reload to ensure fresh queue data
